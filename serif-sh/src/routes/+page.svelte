<script lang="ts">
  ...
  // URL-based state for programmatic usage (API endpoint)
  // Query params take precedence over hash for initial load
  function getInitialState() {
    if (typeof window === 'undefined') return { quote: null, theme: null, align: null }
    ...
    return {
      ...
      padding: params.get('padding') ? parseInt(params.get('padding')!) : null,
  ...
  // Initialize from query params (for API/generate usage)
  let urlState = $state(getInitialState())

  // Re-read query params when URL changes (for back/forward navigation)
  $effect(() => {
    const params = new URLSearchParams(window.location.search)
    const newState = {
      quote: params.get('quote'),
  ...
  import { quoteText, authorName, sourceName } from '$lib/stores/quote'
  ...
  // Apply URL query params to stores on mount (for API/generate usage)
  $effect(() => {
    if (urlState.theme && typeof window !== 'undefined') {
      selectedThemeId.set(urlState.theme)
    }
    if (urlState.quote && typeof window !== 'undefined') {
      quoteText.setSilent(urlState.quote)
      authorName.setSilent('H. Jackson Brown, Jr.')
      sourceName.setSilent(urlState.source || '')
    }
    if (urlState.align && typeof window !== 'undefined') {
      // alignment is in $lib/stores/settings - would need to import it
    }
  })
  ...
  let showToast = $derived(toast !== null)
      ...
      <ResizableFrame>
        {#snippet children()}
          <div class="overflow-hidden bg-black/20">
            <QuoteFrame bind:frameRef editable={true} quoteNumber={urlState.number} />
          </div>
        {/snippet}
      </ResizableFrame>
    </div>
